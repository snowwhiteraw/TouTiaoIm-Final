...
protocol PageBarDelegate: NSObjectProtocol {
    func pageBar(bar: PageBar, willSelectItemAt index: Int) -> Bool
    /// isReload = true 时为点击当前选中的item，触发刷新
    func pageBar(bar: PageBar, didSelectedItemAt index: Int, isReload: Bool) -> Void
}

enum PageSelectionLineViewMode {
...
class PageBar: UIView {
    weak var delegate: PageBarDelegate?
    ...
    lazy var lineViewMode: PageSelectionLineViewMode = PageSelectionLineViewMode.resizesToTextWidth
...
//标题颜色
    var itemTitleColor: UIColor = UIColor.systemPink {
        didSet {
            for item in items {
 ...
 //选中的标题颜色
    var itemTitleSelectedColor: UIColor = UIColor.red {
        didSet {
            for item in items {
...
//选中的标题字体
    var itemTitleSelectedFont: UIFont? = UIFont.systemFont(ofSize: 20) {
        didSet {
...
//                item.layer.borderColor = UIColor.blueColor().CGColor
    ...
    @objc func pageItemClicked(_ item: PageItem) {
...
extension PageBar: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        var leftItemOptional: PageItem?
        var rightItemOptional: PageItem?
